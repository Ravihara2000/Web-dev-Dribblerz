...
let answers = ["Warriors","Steph Curry","Wilt Chamberlain","Bill Russel","Chicago Bulls","Derrick Rose","30","Steph Curry","Los Angeles Lakers","Steph Curry"];
...
function results(){	     
    ...
    document.getElementById("seconds").innerHTML = eliminated_time;
	...
	if(points>0){
		document.getElementById("results").style.boxShadow = "0 8px 32px 0 green";
	}
	else{
		document.getElementById("results").style.boxShadow = "0 8px 32px 0 red";
	}
}

let eliminated_time = 0;


function countdown(){	  /* function to show the timer */
    let timeleft = 60;

    updateTimer();

    function twoDigits( n ){  /* using 2-digits for the timer */	
    	let t;
    	if(n <= 9 ){
    		t = "0" + n;
    	}
    ...
    function updateTimer(){ /* function to update the timer every second */
        timeleft--;
        if ( timeleft < 0 ) {
            ...
            document.getElementById("timer").innerHTML =  '00:' + twoDigits(timeleft);
            eliminated_time++;
